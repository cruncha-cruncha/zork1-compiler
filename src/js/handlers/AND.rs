...
impl HandleJS for AND {
    fn validate (root: &Node) -> Result<(), HandlerErr> {
        if !root.is_routine() ||
           root.children.len() < 3 ||
           !root.children[0].is_word() ||
           root.children[0].tokens[0].value != "AND" {
            return Err(HandlerErr::origin(format!("Invalid AND: {}", root)));
        }
        Ok(())
    }
    ...
    fn print(root: &Node, indent: u64, mut writer: &mut CustomBufWriter<File>) -> Result<(), OutputErr> {
        ...
        for i in 1..root.children.len() {
            match root.children[i].kind() {
                ...
                _ => return Err(OutputErr::from(HandlerErr::origin("Cannot print unknown NodeType in AND"))),
            };
